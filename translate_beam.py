...
def get_args():
    ...
    parser.add_argument('--alpha',default=0.6, type=float, help='length normalization rate') # args.alpha
    parser.add_argument('--gamma',default=1,type=float, help='low-rank hypotheses penalty rate') # args.gamma
...
def main(args):
    ...
    for i, sample in enumerate(progress_bar):
        # print('sample is: ', sample['id'].data)
        ...
        for w in range(args.max_len-1): 

            # Get the current nodes to expand
            nodes = [n[1] for s in searches for n in s.get_current_beams()] # get the node of lowest negative log-prob for each word in the sentence
            if nodes == []:
                break # All beams ended in EOS

            # Reconstruct prev_words, encoder_out from current beam search nodes
            prev_words = torch.stack([node.sequence for node in nodes]) #padding with <unk> to yield same length
            ...
            for i in range(log_probs.shape[0]):
                for j in range(args.beam_size): 
                    ...
                    # see __QUESTION 2
                    best_candidate = next_candidates[i, :, j] # rank of current candidate among siblings: j+1
                    backoff_candidate = next_candidates[i, :, j+1]  
                    best_log_p = log_probs[i, :, j]
                    backoff_log_p = log_probs[i, :, j+1]
                    next_word = torch.where(best_candidate == tgt_dict.unk_idx, backoff_candidate, best_candidate)
                    log_p = torch.where(best_candidate == tgt_dict.unk_idx, backoff_log_p-args.gamma*(j+1), best_log_p-args.gamma*j)
                    log_p = log_p[-1]
                    ...
                    length_norm_0 = ((5+w+1)**args.alpha) / ((5+1)**args.alpha)
                    length_norm_1 = ((5+w+2)**args.alpha) / ((5+1)**args.alpha)


                    if next_word[-1 ] == tgt_dict.eos_idx:
                        node = BeamSearchNode(search, node.emb, node.lstm_out, node.final_hidden,
                                              node.final_cell, node.mask, torch.cat((prev_words[i][0].view([1]),
                                              next_word)), node.logp/length_norm_0, node.length)
                        search.add_final(-node.eval(), node)

                    # Add the node to current nodes for next iteration
                    else:
                        if w+1 == args.max_len-1:
                            node = BeamSearchNode(search, node.emb, node.lstm_out, node.final_hidden,
                                              node.final_cell, node.mask, torch.cat((prev_words[i][0].view([1]),
                                              next_word)), (node.logp + log_p)/length_norm_1, node.length + 1)
                        else: 
                            node = BeamSearchNode(search, node.emb, node.lstm_out, node.final_hidden,
                                              node.final_cell, node.mask, torch.cat((prev_words[i][0].view([1]),
        ...
        best_sents = torch.stack([n[1].sequence[1:] for search in searches for n in search.get_best()] )
        ...
        for sent in output_sentences:
            first_eos = np.where(sent == tgt_dict.eos_idx)[0] #return indices of elemnets satisfying the condition
            if len(first_eos) > 0:
        ...
        for ii, sent in enumerate(output_sentences):
            all_hyps[ii+3*int(sample['id'].data[0])] = sent
